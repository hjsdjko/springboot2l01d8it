...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="成色要求" prop="chengseyaoqiu">
            <el-select v-model="ruleForm.chengseyaoqiu" placeholder="请选择成色要求" :disabled=" false  ||ro.chengseyaoqiu" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="接受价位" prop="jieshoujiawei">
            <el-input v-model="ruleForm.jieshoujiawei" 
                placeholder="接受价位" clearable :disabled=" false  ||ro.jieshoujiawei"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="需求数量" prop="xuqiushuliang">
            <el-input v-model.number="ruleForm.xuqiushuliang" 
                placeholder="需求数量" clearable :disabled=" false  ||ro.xuqiushuliang"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`qiugoushangpin/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('qiugoushangpin/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`qiugoushangpin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
