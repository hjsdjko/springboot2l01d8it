			...
			this.$refs["ruleForm"].validate(async valid => {
				if (valid) {
					...
					if (this.$storage.get("sessionTable") == 'yonghu') {
						if(this.ruleForm.newpassword && (this.ruleForm.newpassword.length<6)){
							this.$message.error(`新密码长度不能小于6`);
							return
						}
						if(this.ruleForm.newpassword && (this.ruleForm.newpassword.length>12)){
							this.$message.error(`新密码长度不能大于12`);
							return
						}
					}
					this.user.password = this.ruleForm.newpassword;
